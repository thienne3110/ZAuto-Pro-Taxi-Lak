...
public class ZaloWebManager {
    ...
    public static void initWebView(Activity activity) {
        activity.runOnUiThread(() -> {
            ...
            // Thêm layout vào Activity nhưng ban đầu cấu hình ẩn và kích thước 0
    ...
    // =========================================================
    // HÀM ĐƯỢC PYTHON GỌI ĐỂ DI CHUYỂN WEBVIEW VÀO CONTAINER KIVY
    // =========================================================
    public static void updateWebViewBounds(Activity activity, int x, int y, int width, int height, boolean visible) {
        activity.runOnUiThread(() -> {
            if (webLayout != null) {
                if (!visible) {
                    webLayout.setVisibility(android.view.View.GONE);
    ...
    // =========================================================
    // HÀM BẮN NỘI DUNG CHỐT (QUAN TRỌNG CHỐNG CRASH)
    // =========================================================
    public static void sendReply(Activity activity, String groupName, String replyText) {
        String js = "window.sendHiddenMessage('" + replyText + "');";
        executeJS(activity, js);
    }

    public static void reloadWeb(Activity activity) {
        activity.runOnUiThread(() -> {
