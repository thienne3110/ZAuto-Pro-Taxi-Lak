    ...
    public void executeReplyContext(final String groupName, final String replyText) {
        executor.execute(() -> {
            ...
            try {
                ...
                String currentPkg = root.getPackageName().toString();
                if (!currentPkg.equals(ZALO_PACKAGE)) return;
        ...
        while (!queue.isEmpty()) {
            ...
            try {
                ...
                if (txt != null) {
                    ...
                    if (t.contains(target.toLowerCase())) return node;
                }
        ...
        while (!queue.isEmpty()) {
            ...
            try {
                ...
                if (desc != null) {
                    ...
                    if (d.contains("gửi") || d.contains("send")) return node;
                }
                if (txt != null) {
                    String t = txt.toString().toLowerCase();
                    if (t.equals("gửi") || t.equals("send")) return node;
                }
                for (int i = 0; i < node.getChildCount(); i++) {
                    AccessibilityNodeInfo child = node.getChild(i);
                    if (child != null) queue.add(child);
